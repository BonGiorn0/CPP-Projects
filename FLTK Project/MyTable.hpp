...
class MyTable : public Fl_Table {
  ...
  //
  void DrawData(int n, int X, int Y, int W, int H) {
    fl_push_clip(X,Y,W,H);
      // Draw cell bg
      if (n > 0){
        fl_color(FL_WHITE);
...
public:
  ...
  size_t getValue(int row, int col) const{
    return data[row][col];
  }

  void setRowCount(size_t val){
    if(val > MAX_ROWS)
      rowCount = MAX_ROWS;
    else
      rowCount = val;
  ...
  void Resize(size_t width, size_t height){
    rows(rowCount);
    row_height_all(width / rowCount);
    cols(colCount);
    col_width_all(height / colCount);
  }
